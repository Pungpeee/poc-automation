...
const MarketPlaceExContainer = () => {
  const [testq,settest] = useState([])
  const { contractdata, res, nftData } = useNFTMarketplace()
  const axios = require("axios")

  useEffect(()=>{
    const test = useQueryNFT({
      options: {
        onSuccess: (s) => {
          s.data.contract_list.map((d) => {
            axios
              .post(
                "http://18.141.49.87:3001/v2/nft/nft_owner_info",
                {
                  contract: d.contract,
                },
                {
                  headers: {
                    authorization: "e76e71db5152ca53e576b0cc55e71fff",
                    "Content-Type": "application/json",
                  },
                }
              )
              .then((nftdata) => {
                nftdata.data.nft_list.results.map((a) => {
                  axios.get(a).then((b) => {
                    testq.push(b.data)
                  })
                })
  ...
  return (
    <Box height="100%">
      <MainContainer>
        ...
        <TopCollection>
          {contractdata?.data?.contract_list.map((d) => {
...
const Collections_display = (props) => {
  ...
  return (
    <Box display="flex" flex="0 0 16%" flexDirection="column" alignItems="center" justifyContent="center">
      <Box width="150px" height="150px" sx={{ borderRadius: "50%" }}>
