...
const MOCK_THEMALL = [
  { label: "เดอะมอลล์ รามคำแหง", stock: 99 },
  { label: "เดอะมอลล์ บางแค", stock: 99 },
  { label: "เดอะมอลล์ บางกะปิ", stock: 99 },
  { label: "เดอะมอลล์ นครราชสีมา", stock: 99 },
  { label: "เดอะมอลล์ไลฟ์สโตร์ ท่าพระ", stock: 99 },
  { label: "เดอะมอลล์ไลฟ์สโตร์ งามวงศ์วาน", stock: 99 },
  { label: "ดิ เอ็มโพเรียม", stock: 99 },
  { label: "ดิ เอ็มควอเทียร์", stock: 99 },
  { label: "พารากอน", stock: 99 },
]

const NFTPremiumContainer = () => {
  ...
  const ClaimNFT = async (data) => {
    const res = await CustomClient.post("/nft/claim_reward/premium", data)
    return res.data
  }
  ...
  const handleClaim = () => {
    ...
    } else if (dataNFT.category === "premium") {
      const payload1 = {
        price: dataNFT.price * 10,
        type: "premium",
        brand: dataNFT.brand,
      ...
      {isSuccess && (
        <MoreDetailWrapper>
          ...
          <MoreDetailComponentWrapper>
            ...
            {seeAll && (
              <TheMallListBox>
                {MOCK_THEMALL?.map(({ label, stock }) => {
                  return (
                    <TheMallList_Wrapper
                      label={label}
                      branch={branch}
                      onClick={() => {
                        handleClickBranch(label)
                      }}
                    >
                      <Text type="12" fontWeight={400} color={branch === label ? "white" : "black"}>
                        {label}
                      </Text>
                      <Text type="12" fontWeight={400} color={branch === label ? "white" : "black"}>
                        {`จำนวนคงเหลือ ${stock}`}
            ...
            {isSlide && (
              <CodeBox>
                <CodeWrapper>
                  <Text type="18" fontWeight={400} color="black">
                    {data.barcode_id ?? 'AFFFF'}
                  </Text>
                </CodeWrapper>
                <TimeBox>
...
export default NFTPremiumContainer
...
const TheMallList_Wrapper = styled(Box)(({ branch, label }) => ({
  minWidth: " 343px",
  maxWidth: "100%",
  height: " 36px",
  background: !branch ? "rgba(31, 163, 124, 0.1)" : branch === label ? "#34A182" : "rgba(31, 163, 124, 0.1)",
  borderRadius: "8px",
  display: "flex",
  padding: "0px 16px",
